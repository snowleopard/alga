...
import Algebra.Graph.NonEmpty.AdjacencyIntMap
...
testNonEmptyAdjacencyIntMap = do
    ...
    test "show (1 + 2     :: AdjacencyIntMap) == \"vertices1 [1,2]\"" $
          show (1 + 2     :: AdjacencyIntMap) == "vertices1 [1,2]"

    test "show (1 * 2     :: AdjacencyIntMap) == \"edge 1 2\"" $
          show (1 * 2     :: AdjacencyIntMap) == "edge 1 2"

    test "show (1 * 2 * 3 :: AdjacencyIntMap) == \"edges1 [(1,2),(1,3),(2,3)]\"" $
          show (1 * 2 * 3 :: AdjacencyIntMap) == "edges1 [(1,2),(1,3),(2,3)]"

    test "show (1 * 2 + 3 :: AdjacencyIntMap) == \"overlay (vertex 3) (edge 1 2)\"" $
          show (1 * 2 + 3 :: AdjacencyIntMap) == "overlay (vertex 3) (edge 1 2)"

    test "show (vertex (-1)                             :: AdjacencyIntMap) == \"vertex (-1)\"" $
          show (vertex (-1)                             :: AdjacencyIntMap) == "vertex (-1)"

    test "show (vertex (-1) + vertex (-2)               :: AdjacencyIntMap) == \"vertices1 [-2,-1]\"" $
          show (vertex (-1) + vertex (-2)               :: AdjacencyIntMap) == "vertices1 [-2,-1]"

    test "show (vertex (-1) * vertex (-2)               :: AdjacencyIntMap) == \"edge (-1) (-2)\"" $
          show (vertex (-1) * vertex (-2)               :: AdjacencyIntMap) == "edge (-1) (-2)"

    test "show (vertex (-1) * vertex (-2) * vertex (-3) :: AdjacencyIntMap) == \"edges1 [(-2,-3),(-1,-3),(-1,-2)]\"" $
          show (vertex (-1) * vertex (-2) * vertex (-3) :: AdjacencyIntMap) == "edges1 [(-2,-3),(-1,-3),(-1,-2)]"
    ...
    test "preIntSet 1 (edge 1 2) == IntSet.empty" $
          preIntSet 1 (edge 1 2 :: G) == IntSet.empty
    ...
    test "postIntSet x (edge x y) == IntSet.fromList [y]" $ \(x :: Int) y ->
...
--          postIntSet x (closure y)   == IntSet.fromList (reachable x y)
    ...
    test "symmetricClosure (edge x y)         == edges1 [(x,y), (y,x)]" $ \(x :: Int) y ->
    ...
    test "transitiveClosure (edge x y)          == edge x y" $ \(x :: Int) y ->
